...
.nav-left { 
    font-weight:900; font-size:2.2rem; letter-spacing:2px; 
    color:#fff; text-shadow:0 0 25px #00cfff,0 0 35px #00eaff; 
    animation: neonPulse 2s ease-in-out infinite;
}
@keyframes neonPulse { 0%,100%{text-shadow:0 0 25px #00cfff,0 0 35px #00eaff;} 50%{text-shadow:0 0 35px #00eaff,0 0 55px #00cfff;} }

.menu-liste a { 
    position:relative; color:#fff; font-weight:700; font-size:1.3rem; padding-bottom:6px; 
    transition:0.3s ease, transform 0.3s ease; 
}
.menu-liste a:hover { transform:translateY(-3px); color:#00eaff; text-shadow:0 0 12px #00cfff; }
.menu-liste a::after { content:""; position:absolute; left:0; bottom:-4px; width:0%; height:3px; background:#00cfff; border-radius:10px; transition:width 0.3s ease; }
...
.boutonCoDeco a { 
    ...
    color:#00cfff; padding:0.5rem 1.5rem; border-radius:2rem; backdrop-filter:blur(4px); 
    transition:0.3s; 
}
.boutonCoDeco a:hover { transform:scale(1.05); box-shadow:0 0 20px #00eaff,0 0 35px #00cfff; }

/* === TITRE PRINCIPAL === */
.big-title { font-size:3rem; text-align:center; font-weight:900; color:#00cfff; 
...
@keyframes glowTitle { 0%{text-shadow:0 0 25px #00cfff,0 0 45px #00eaff;} 50%{text-shadow:0 0 35px #00eaff,0 0 60px #00cfff;} 100%{text-shadow:0 0 25px #00cfff,0 0 45px #00eaff;} }

/* === CARDS === */
.sunset-card { background: linear-gradient(135deg,#00141a,#002733,#00141a); border-radius:1.5rem; padding:1rem; border:2px solid #00cfff; text-align:center; box-shadow:0 0 15px rgba(0,255,255,0.25); display:flex; flex-direction:column; height:100%; transition:0.5s; transform: perspective(1000px) rotateY(0deg); opacity:0; animation: fadeInCard 0.8s forwards; }
.sunset-card:hover { transform: perspective(1000px) rotateY(10deg) translateY(-10px); box-shadow:0 0 35px #00eaff,0 0 60px #00cfff; }
@keyframes fadeInCard { 0%{opacity:0; transform:translateY(30px);} 100%{opacity:1; transform:translateY(0);} }

.card-inner { background:rgba(0,0,0,0.65); border:1px solid #00a2c9; border-radius:1rem; padding:1.5rem; color:#c8f7ff; display:flex; flex-direction:column; justify-content:space-between; height:100%; gap:1rem; transition:0.3s; }
.card-inner:hover { transform: scale(1.02); }
.card-title { font-size:1.8rem; font-weight:800; color:#00cfff; text-shadow:0 0 12px #00eaff; animation: titlePop 0.8s ease forwards; }
...
<nav>
  ...
  <ul class="menu-liste flex gap-8">
    ...
    <li><a href="<?= site_url('concerts') ?>" class="active">Concerts</a></li>
...
<div class="fond-blanc-etendu pt-32">
  <h1 class="big-title">Les concerts</h1>
      ...
      <div class="sunset-card h-full">
        <div class="card-inner">
          <div>
            ...
            <p class="card-desc"><strong>Début :</strong> <?= date('d/m/Y H:i', strtotime($event['date_heure_debut'])) ?><br><strong>Lieu :</strong> <?= ucfirst(esc($event['lieu'])) ?></p>
          </div>
